...
{ //Запуск будет произведен на 4 процессах
  //Этот файл разбивает формулу на части и каждый процесс вычисляет собственную часть формулы
    ...
    double sum,time,totaltime;        //и середение которых вычисляются значения функций для полиноминальной функции
    MPI_Status st;
    MPI_Init(&argc,&argv);
    ...
    case 0:
        ...
        for(int i = 1;i<=3;i++)
            {
                MPI_Recv(&result,1,MPI_DOUBLE,MPI_ANY_SOURCE,666,MPI_COMM_WORLD,&st);
                sum += result;
                MPI_Recv(&time,1,MPI_DOUBLE,MPI_ANY_SOURCE,13,MPI_COMM_WORLD,&st);
                totaltime += time;
            }
            sum = h*sum/3;
         int fn = MPI_Wtime();
    ...
    case 1:                             //первый процесс вычисляет сумму значений на концах общего интервала;
        ...
        fprintf(file,"Файл процесса %d",procid);
        fprintf(file,"Время выполенения на процессе %d : %f",procid,totaltime);
        fclose(file);
        break;
